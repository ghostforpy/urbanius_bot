...
def broadcast_decision_handler(update, context): #callback_data: CONFIRM_DECLINE_BROADCAST variable from manage_data.py
    ...
    entities_for_celer = update.callback_query.message.to_dict().get('entities')
    ...
    if broadcast_decision == md.CONFIRM_BROADCAST:
        ...
        #broadcast_message.delay(user_ids=user_ids, message=text, entities=entities_for_celer)
    else:
        admin_text = text
